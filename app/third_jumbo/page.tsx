...
export default function ShowThirdJumbo() {
  return (
    <section id="three">
      <div
        className="flex flex-col px-50 py-30 mb-15 bg-cover"
        style={{ backgroundImage: "url('./sample_bg.png')" }}
      >
        <h4 className="text-white text-4xl font-bold">
          My Recent{" "}
          <span className="text-[#00ADB5] text-4xl font-bold">Works</span>
        </h4>
        *
        <div className="flex flex-row gap-5">
          {variants.map((e) => (
            <h4
              key={e.id}
              className="bg-[#00ADB5] text-white font-bold px-5 py-1 rounded-2xl"
            >
              {e.category}
            </h4>
          ))}
